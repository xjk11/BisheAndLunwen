  ...
  <div class="app-container">
    ...
    <el-row :gutter="10" class="mb8">
      <el-col :span="1.5">
        <el-button
          ...
          icon="Plus"
          @click="handleAdd"
          v-hasPermi="['education:userExam:add']"
        >新增</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button
          ...
          :disabled="single"
          @click="handleUpdate"
          v-hasPermi="['education:userExam:edit']"
        >修改</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button
          ...
          :disabled="multiple"
          @click="handleDelete"
          v-hasPermi="['education:userExam:remove']"
        >删除</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button
          ...
          icon="Download"
          @click="handleExport"
          v-hasPermi="['education:userExam:export']"
        >导出</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button
    ...
    <el-table v-loading="loading" :data="userExamList" @selection-change="handleSelectionChange">
      ...
      <el-table-column label="操作" align="center" class-name="small-padding fixed-width">
        <template #default="scope">
          <el-button link type="primary" icon="Edit" @click="handleUpdate(scope.row)" v-hasPermi="['education:userExam:edit']">修改</el-button>
          <el-button link type="primary" icon="Delete" @click="handleDelete(scope.row)" v-hasPermi="['education:userExam:remove']">删除</el-button>
        </template>
      </el-table-column>
    </el-table>
    ...
    <el-dialog :title="title" v-model="open" width="500px" append-to-body>
      <el-form ref="userExamRef" :model="form" :rules="rules" label-width="80px">
...
import { listUserExam, getUserExam, delUserExam, addUserExam, updateUserExam } from "@/api/education/userExam";
import { getToken } from "@/utils/auth";

const { proxy } = getCurrentInstance();
const { course_duration } = proxy.useDict('course_duration');

const userExamList = ref([]);
...
function getList() {
  loading.value = true;
  listUserExam(queryParams.value).then(response => {
    userExamList.value = response.rows;
...
function handleSelectionChange(selection) {
  ids.value = selection.map(item => item.userExamId);
...
function handleAdd() {
  reset();
  open.value = true;
  title.value = "添加个人考试信息查询";
}

/** 修改按钮操作 */
function handleUpdate(row) {
  reset();
  const _userExamId = row.userExamId || ids.value
  getUserExam(_userExamId).then(response => {
    form.value = response.data;
    open.value = true;
    title.value = "修改个人考试信息查询";
  });
}

/** 提交按钮 */
function submitForm() {
  proxy.$refs["userExamRef"].validate(valid => {
    if (valid) {
      if (form.value.userExamId != null) {
        updateUserExam(form.value).then(response => {
          proxy.$modal.msgSuccess("修改成功");
          open.value = false;
          getList();
        });
      } else {
        addUserExam(form.value).then(response => {
...
function handleDelete(row) {
  const _userExamIds = row.userExamId || ids.value;
  proxy.$modal.confirm('是否确认删除考试信息用户信息关联编号为"' + _userExamIds + '"的数据项？').then(function() {
    return delUserExam(_userExamIds);
  }).then(() => {
    getList();
...
function handleExport() {
  proxy.download('education/userExam/export', {
    ...queryParams.value
...
// 上传地址
const uploadExcelUrl = ref(import.meta.env.VITE_APP_BASE_API + "/education/userExam/import");
// 上传请求头
...
function importExcelModel() {
  proxy.download('education/userExam/exportModel', {
  }, `userExam_${new Date().getTime()}.xlsx`)
}
