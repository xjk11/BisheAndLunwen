  ...
  <div class="app-container">
    <el-form :model="queryParams" ref="queryRef" :inline="true" v-show="showSearch" label-width="68px">
      ...
      <el-form-item label="上课时间" prop="lessonTime">
        <el-select v-model="queryParams.lessonTime" placeholder="请选择上课时间" clearable>
          <el-option
            v-for="dict in lesson_time"
            :key="dict.value"
            :label="dict.label"
            :value="dict.value"
          />
    ...
    <el-row :gutter="10" class="mb8">
      <el-col :span="1.5">
        <el-button
          ...
          icon="Plus"
          @click="handleAdd"
          v-hasPermi="['education:schedule:add']"
        >新增</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button
          ...
          :disabled="single"
          @click="handleUpdate"
          v-hasPermi="['education:schedule:edit']"
        >修改</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button
          ...
          :disabled="multiple"
          @click="handleDelete"
          v-hasPermi="['education:schedule:remove']"
        >删除</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button
          ...
          icon="Download"
          @click="handleExport"
          v-hasPermi="['education:schedule:export']"
        >导出</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button
    ...
    <el-table v-loading="loading" :data="scheduleList" @selection-change="handleSelectionChange">
      ...
      <!-- <el-table-column label="课程安排id" align="center" prop="scheduleId" /> -->
      ...
      <el-table-column label="上课时间" align="center" prop="lessonTime">
        <template #default="scope">
          <dict-tag :options="lesson_time" :value="scope.row.lessonTime"/>
        </template>
      </el-table-column>
      ...
      <el-table-column label="周几上课" align="center" prop="dayOfWeek" />
      <el-table-column label="操作" align="center" class-name="small-padding fixed-width">
        <template #default="scope">
          <el-button link type="primary" icon="Edit" @click="handleUpdate(scope.row)" v-hasPermi="['education:schedule:edit']">修改</el-button>
          <el-button link type="primary" icon="Delete" @click="handleDelete(scope.row)" v-hasPermi="['education:schedule:remove']">删除</el-button>
        </template>
      </el-table-column>
    </el-table>
    ...
    <el-dialog :title="title" v-model="open" width="500px" append-to-body>
      <el-form ref="scheduleRef" :model="form" :rules="rules" label-width="80px">
        ...
        <el-form-item label="上课时间" prop="lessonTime">
          <el-select v-model="form.lessonTime" placeholder="请选择上课时间">
            <el-option
              v-for="dict in lesson_time"
              :key="dict.value"
              :label="dict.label"
        ...
        <el-form-item label="星期几" prop="dayOfWeek">
          <el-input v-model="form.dayOfWeek" placeholder="请输入周几上课" />
        </el-form-item>
      </el-form>
      <template #footer>
...
import { listSchedule, getSchedule, delSchedule, addSchedule, updateSchedule } from "@/api/education/schedule";
import { getToken } from "@/utils/auth";

const { proxy } = getCurrentInstance();
const { lesson_time, course_duration } = proxy.useDict('lesson_time', 'course_duration');
...
const data = reactive({
  ...
  rules: {
    userId: [
      { required: true, message: "用户ID，外键关联 sys_user 表不能为空", trigger: "blur" }
    ],
    courseId: [
      { required: true, message: "课程ID，外键关联 edu_course 表不能为空", trigger: "blur" }
    ],
    teacherId: [
      { required: true, message: "教师ID，外键关联 edu_teachers 表不能为空", trigger: "blur" }
    ],
    classroom: [
    ...
    startTime: [
      { required: true, message: "开课学年 值为1到6，从2023开始不能为空", trigger: "change" }
    ],
    dayOfWeek: [
...
function handleDelete(row) {
  ...
  proxy.$modal.confirm('是否确认删除个人课程编号为"' + _scheduleIds + '"的数据项？').then(function() {
...
function handleExport() {
  proxy.download('education/schedule/export', {
    ...queryParams.value
...
// 上传地址
const uploadExcelUrl = ref(import.meta.env.VITE_APP_BASE_API + "/education/schedule/import");
// 上传请求头
...
function importExcelModel() {
  proxy.download('education/schedule/exportModel', {
  }, `schedule_${new Date().getTime()}.xlsx`)
}
